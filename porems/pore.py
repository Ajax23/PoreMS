...
class Pore(Molecule):
    ...
    def _special(self,mol,sio,num,symmetry):
        ...
        * **mirror** - Mirror symmetry
        ...
        if symmetry not in ["random","point","mirror"]:
            ...
            for i in range(num):
                if   symmetry=="point":  coeff = -1 if i%2==0 else 1
                elif symmetry=="mirror": coeff =  1
    ...
    def _excess(self,is_rand=True):
        ...
        for mol in mols:
            ...
            if   mol.get_name()=="om":   num_o  += 1
            elif mol.get_name()=="si":   num_si += 1
            elif mol.get_name() in grid: num_g  += 1
        ...
        # Distribute charge
        for mol in mols:
            if   mol.get_name()=="om":   mol.set_charge(q_o)
            elif mol.get_name()=="si":   mol.set_charge(q_si)
            elif mol.get_name() in grid: mol.set_charge(round(mol.get_charge()-self._q_si+q_si,round_o))
        ...
        if is_rand:
            ...
            write    = []
            write_ox = []
            for i in range(min_o): write.append(mols[i])
            for i in range(min_o,max_o+1):
                if i not in oxy_r: write.append(mols[i])
                else: write_ox.append(mols[i])
            write += write_ox
            for i in range(max_o+1,len(mols)): write.append(mols[i])
    ...
    def coupleProxi(self,mol,sio,orient,rate,inp="percent"):
        ...
        """
        t = utils.tic()
        if mol is not None: self._couple_proxi(mol,sio,orient,rate,inp=inp)
        self._t_tot += utils.toc(t,"Couple ",self._is_time)
    ...
    def finish(self,slx=0,is_rand=True,is_mol=False,is_props=True):
        ...
        if is_props:
            ...
            # Dimensions
            if   drill=="x": size_str = "%4.1f"%size[2]+" "+"%4.1f"%size[1]+" "+"%4.1f"%size[0]
            elif drill=="y": size_str = "%4.1f"%size[0]+" "+"%4.1f"%size[2]+" "+"%4.1f"%size[1]
            elif drill=="z": size_str = "%4.1f"%size[0]+" "+"%4.1f"%size[1]+" "+"%4.1f"%size[2]
